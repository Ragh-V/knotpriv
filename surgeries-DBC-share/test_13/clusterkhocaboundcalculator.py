...
def main():
    # Removed the /workspace/ prefix so it reads/writes in your current cluster directory
    input_file = 'DTList.txt'
    output_file = 'HFBoundsKhoca.txt'
    ...
    with open(output_file, 'w') as f_out, open(profile_file, 'w') as f_prof:
        ...
        for raw_code in tqdm(lines, desc="Processing Knots", unit="knot"):
            ...
            # Updated to use the absolute path to khoca.py at the container's root
            cmd = ["/khoca/khoca.py", "2", "0.0", "0", pd_code, "calc0"]
