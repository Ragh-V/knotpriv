...
# --------------------------------------------------------------------
# Exact sawtooth and Dedekind sum (Copied directly from casson_only.py)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Attach HF data from text files
# --------------------------------------------------------------------
...
def run_hf_casson(records: List[KnotRecord]):
    ...
    for k, rec in enumerate(records):
        ...
        # Strictly use sympy.Rational to match casson_only.py
        cassonDBC = sym.Rational(str(rec.casson_dbc))
        aval      = sym.Rational(str(rec.A))
        ...
        if mini <= HFbound:
            ...
            for q in [qq for qq in range(q_lo, q_hi+1) if (qq != 0 and gcd(qq, p) == 1)]:
                # Compute exact Dedekind sum and surgery invariant strictly using SymPy
                qR = sym.Rational(q)
                s_qp = dedekind_sum(q, p)
                
                cassonSurgery = qR * aval / p - s_qp / sym.Rational(2)
                cassonSurgery = sym.simplify(cassonSurgery)
                
                computed_cassons.append(f"q={q}: {cassonSurgery}")
                
                # Compare absolute values safely
                if sym.Abs(cassonSurgery) == sym.Abs(cassonDBC):
